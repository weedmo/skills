...
- **Codex** — `spawn_agent` by role name (`matt-default` / `matt-deep` / `matt-deep-max`; autocode `experimenter-default` / `experimenter-deep` / `strategist` / `strategist-max`), `fork_turns: "none"`, every required path, spec, constraint, and `ROUTED_EXECUTION=1`. Role files (`~/.codex/agents/`) fix model, effort, standard tier, and Default's low verbosity; missing role → the table's pair, reported once. `send_message` updates running agents; `followup_task` starts another turn after completion.
