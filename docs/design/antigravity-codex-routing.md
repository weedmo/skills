...
4. `skills/design-map/SKILL.md`의 6단계 표 `direct` 행이 "3–6단계, 테스트 적음 → `direct`; `[deep]` 있으면 Astra/Fable, 전부 `[default]`면 Antigravity"이고, 8단계에 antigravity 분기(`--command agy`, 프롬프트 감지, 60초 초과 시 명령만 출력)가 있다.
...
| id | 질문 | 선택 | 이유 |
|---|---|---|---|
| E1 | Orca가 antigravity CLI를 터미널로 구동할 수 있나? | **probe 결과(2026-09-07)**: CLI는 `agy`(`~/.local/bin/agy`, Antigravity CLI 1.1.27; `antigravity`라는 바이너리는 없음). `orca terminal create --command agy`·`read --screen`·`close` 모두 성공. 첫 화면은 폴더 신뢰 대화상자(`Do you trust the contents of this project?` / `> Yes, I trust this folder`, Enter로 확인)이고 상태줄에 `Gemini 3.8 Flash · high`가 보인다. 신뢰 뒤의 idle 프롬프트 문자열은 이 probe에서 수락하지 않아 미기록 — design-map 8단계는 "신뢰 대화상자면 Enter, 그 뒤 프롬프트 또는 60초 안의 화면 변화"로 둔다 | Orca와 CLI 모두 확인; 프롬프트 문자열만 사용자가 한 번 신뢰한 뒤 기록 |
...
| E3 | `service_tier`의 standard 값, 역할 파일의 키 지원, priority 배수는? | **probe 결과(2026-09-07, Codex 0.153.4)**: 역할 파일의 `service_tier = "default"`는 `codex --strict-config exec`에서 거부되지 않았고 `matt-default` 스폰이 정상 동작. 실제 적용 여부는 세션 로그(`~/.codex/sessions`)에 tier가 기록되지 않아 확인 불가 — 키는 유지하고 README에 "수용됨, 적용은 미확인"으로 적음. priority 배수는 여전히 미확인 | 거부되지 않는 것까지 확인; 청구서로만 최종 검증 가능 |
| E4 | `model_verbosity`를 역할 파일이 존중하고 terra가 지원하나? | **probe 결과**: `model_verbosity = "low"`도 `--strict-config`에서 거부되지 않음; 로그에 verbosity가 기록되지 않아 적용 여부는 미확인. 키 유지 | E3과 동일 |
| E5 | 하네스 `max` ↔ Codex effort 이름은? | **probe 결과**: `xhigh`가 역할 파일 로드(`--strict-config`)에서 거부되지 않음; max 역할 자체는 스폰하지 않아 astra가 실제로 받는지는 첫 `max` 재시도 때 확인 | 로드 단계까지 확인 |
