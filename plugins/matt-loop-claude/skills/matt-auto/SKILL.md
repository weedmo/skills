...
4. **Interview** — `$grill-with-docs` in a codebase, `$grill-me` otherwise, one question at a time with a recommended answer, each to the delegate; record question → decision + rationale. A question that needs external sources detours via `/deep-research`; one that needs a runnable answer via `$prototype`. Then run `$interview-report` on the log and hold the **interview gate** until the user approves. With `--spec` there is no interview and no gate — design-map's self-grill and the user's confirmation were it: the log opens with the spec's 결정 table as the read-only `design` stage (see `$interview-report`), the interview stage reads `⏭️ design-map spec`, and the run continues to step 5 (`--confirm` restores the gate, presenting only what later stages add). Open questions surface from later stages to the delegate; one that would reverse a spec decision escalates as `design override <D-id>`; the spec file is never edited.
...
**Interview gate (skipped only on the small path and on a `--spec` run without `--confirm`).** Right after step 4's page is generated the pipeline stops until the user answers.
...
When design-map hands over in this session ("지금 이어서 진행할까요?" answered with 이 세션 or `/fork`), start at step 1 with `--spec <the committed spec>`: the delegate fork already holds the design conversation, and the interview and its gate are skipped — the spec was the interview. Nothing else differs.
