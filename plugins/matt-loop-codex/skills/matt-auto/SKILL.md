---
name: matt-auto
description: "Conducts Matt Pocock's full engineering flow — interview, spec, tickets, implementation — for an already-discussed idea. Not for small work: a one-file change under thirty minutes is faster with `$implement` directly. Unattended by default: a decision delegate answers routine questions, unresolved material decisions escalate, and every decision lands in an editable decision-graph page. Prior approval is reused. Flags: `--spec <path>` starts from a confirmed design-map spec; `--confirm` restores the human confirm gates; `--free` free-only OpenCode routing; `--dev` / `--main` / `--pr <base>` also open a PR and shepherd it to merge-ready; `--parallel N` / `--on <env>` override Orca worker concurrency and placement."
...
4. **Interview** — `$grill-with-docs` in a codebase, `$grill-me` otherwise, each unresolved question with a recommended answer to the delegate; record question → decision + rationale; a runnable question detours via `$prototype`. Reuse answers from the conversation and confirmed spec. Then run `$interview-report` on the log and evaluate the **interview gate** below. With `--spec`, the graph's first stage is `design` — the spec's decisions as read-only nodes (see `$interview-report`); a user-requested change becomes an interview-stage `redirect` logged as `design override <D-id>`; the spec file is never edited.
...
After publishing step 4's page, reuse existing implementation approval if no material decision remains unresolved. A confirmed `--spec` supplied for implementation qualifies unless the interview changes its decisions. Record the approval and continue to step 5. Otherwise present the concrete unresolved choices and wait. `--confirm` requests a fresh gate. Silence is never approval.
