...
The user settles the big frame before invoking this. An automatically routed answer delegate handles questions and decisions; unresolved material decisions go to the user. The decision graph accepts user corrections. No PR without a ship flag; independent tickets use Orca workers when available.
...
- The shared weed-harness skills are the runtime: `$model-routing` (tiers), `$loop-report` (page build and delivery, through `$interview-report`), `$loop-gates` (the ship ledger). If one is missing, say `weed-harness 4.x required: <skill> missing` once and use the fallback it names (stop unroutable dispatch; decision log only; no ship ledger). Never reimplement them inline.
...
| `matt-default` | Default | `matt-loop:matt-default` | Small fixes, tests, mechanical conflicts |
...
- **Routing.** Opening the PR → in-session; babysit → Fast; mechanical conflicts → Default; semantic conflicts → Deep. Unavailable routes follow `$model-routing`; never inherit unknown defaults.
