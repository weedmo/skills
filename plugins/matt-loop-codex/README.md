...
- Routing — the authored skills map their roles (matt-default / matt-deep / matt-deep-max) onto the tiers of weed-harness's shared `model-routing` skill, which holds the model/effort pair per platform and the escalation ladder; on Codex that dispatches by role name through `spawn_agent`
- `codex/agents/` — Codex custom agent roles (`matt-default.toml`, `matt-deep.toml`, `matt-deep-max.toml`), installed to `~/.codex/agents/` by the npx installer; each pins model and reasoning effort, and the Default role also pins the standard service tier and low verbosity so a `priority` user session does not leak into background workers
